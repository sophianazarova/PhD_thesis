...
str(ishodnik)
# посчитали по данным 2012-2013 годов что размер 1+ от 1,2 до 1,8 мм. 
...
#рисуем график 
pdf(file="Estuary_N_oneyear.pdf", family="NimbusSan") # указываем шрифт подпией
  plot(seq(as.numeric(min(colnames(mean.oneyear.sqmeter))),as.numeric(max(colnames(mean.oneyear.sqmeter))),1), 
      mean.oneyear.sqmeter[2,], pch=15, type="b", xlab="год", ylab="N1+, экз./кв.м", main="Эстуарий р. Лувеньги",
...
dev.off()
embedFonts("Estuary_N_oneyear.pdf") #встройка шрифтов в файл
...
#пишем в файл
write.table(data.frame(N1age[2:length(N1age)], N_bez_spata[1:length(N_bez_spata)-1]),file="estuary_corr_1_all.csv", sep=";", dec=",")

#рисуем график N1+ & N>8мм(=половозрелые)
pdf(file="Estuary_N1y_N8mm.pdf", family="NimbusSan") # указываем шрифт подпией
plot(y=mean.oneyear.sqmeter[2,], x=colnames(mean.oneyear.sqmeter),pch=15, type="n", main="Эстуарий р. Лувеньги", 
     ylim=c(0, 
...
dev.off()
embedFonts("Estuary_N1y_N8mm.pdf") #встройка шрифтов в файл

#считаем среднюю численность и ошибки особей старше 1+
str(oneyear.sqmeter)
levels(oneyear.sqmeter$oneyear.int)
(old.sqmeter<-subset(oneyear.sqmeter, subset=oneyear.sqmeter$oneyear.int==c("(1.8,8]","(8,18.2]")))
...
pdf(file="Estuary_N1_Nold.pdf", family="NimbusSan") # указываем шрифт подпией
plot(y=mean.oneyear.sqmeter[2,], x=colnames(mean.oneyear.sqmeter),pch=15, type="n", main="Эстуарий р. Лувеньги", 
     ylim=c(0, 
...
dev.off()
embedFonts("Estuary_N1_Nold.pdf") #встройка шрифтов в файл
