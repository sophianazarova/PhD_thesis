...
pdf(file="YuG_N1y_N8mm.pdf", family="NimbusSan") # указываем шрифт подпией
plot(y=mean.oneyear.sqmeter[2,], x=colnames(mean.oneyear.sqmeter),pch=15, type="n", main="Южная губа о.Ряшкова", 
     ylim=c(0, 
...
dev.off()
embedFonts("YuG_N1y_N8mm.pdf") #встройка шрифтов в файл

#считаем среднюю численность и ошибки особей старше 1+
str(oneyear.sqmeter)
levels(oneyear.sqmeter$oneyear.int)
(old.sqmeter<-subset(oneyear.sqmeter, subset=oneyear.sqmeter$oneyear.int==c("(1.9,8]","(8,22.3]")))
...
pdf(file="YuG_N1_Nold.pdf", family="NimbusSan") # указываем шрифт подпией
plot(y=mean.oneyear.sqmeter[2,], x=colnames(mean.oneyear.sqmeter),pch=15, type="n", main="Южная губа о. Ряшкова", 
     ylim=c(0, 
...
dev.off()
embedFonts("YuG_N1_Nold.pdf") #встройка шрифтов в файл
